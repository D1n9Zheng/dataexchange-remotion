...
> 当前状态：S03 为接入能力动画，S04–S07 已接入真实录屏与动画包装，全片 94 秒。S08 监控仍为占位镜头，等待补录；S01、S02、S09、S10 保持已确定版本。
...
中段预览为 `out/RecordingActPreview.mp4`（59 秒）。全片使用 `npm run render:all`，S08 等待补录。

上一版交付包含网页播放格式的 `out/FullPromoVideo_RecordingDraft_1080p.mp4` 与 `out/RecordingActPreview_1080p.mp4`，为 1080p / 60 FPS / H.264 / yuv420p / 无音轨并启用 Fast Start。Remotion 母版保留在 `out/FullPromoVideo_RecordingDraft.mp4`。S08 仍为待替换占位内容。

上述旧版成片尚未包含本次 S03 优化。当前时间轴为 94 秒，S03 检查片段为 `out/S03_Connections_Preview_1080p.mp4`（含 S02 末尾 1 秒、S03 15 秒、S04 开头 2 秒）。完整衔接预览为 `out/S02_S03_Transition_1080p.mp4`（S02 6 秒 + S03 15 秒 + S04 开头 2 秒）；S02 与 S03 共享产品节点，不再展开界面模板。
...
ffmpeg -y -ss 14 -i out/FullPromoVideo_RecordingDraft_1080p.mp4 -t 59 -an -c:v libx264 -preset fast -crf 16 -pix_fmt yuv420p -color_range tv -movflags +faststart out/RecordingActPreview_1080p.mp4
```
